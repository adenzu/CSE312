...
void execve(void entrypoint())
{
    ...
    task->SetPriority(taskManager.GetCurrentTask()->GetPriority() + 1);
    taskManager.AddTask(task);
    waitpid(task->GetID());
    exit();
}

void get_uint_array(int *arr, int size)
{
    char buffer[256];
    printf("Enter ");
    if (size < 256)
    ...
    for (int i = 0, j = 0; i < size; i++)
    {
        ...
        for (; buffer[j] == ' ' || buffer[j] == ','; j++)
            buffer[j] = ' ';
        for (; buffer[j] != ' ' && buffer[j] != ',' && buffer[j] != 0 && buffer[j] != '\n'; j++)
            arr[i] = arr[i] * 10 + buffer[j] - '0';
    }
}
...
void collatz_sequence()
{
    ...
    taskManager.SetPriority(3);
    get_uint(&length);
    taskManager.SetPriority(0);
...
void long_running_program()
{
    ...
    printf("Long running program result: ");
    printfHex32(result);
    printf("\n");
    exit();
}
